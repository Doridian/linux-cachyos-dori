...
_dori_build_hook() {
    ...
    scripts/config --set-str CONFIG_MODULE_SIG_KEY "${sign_key}" -e CONFIG_MODULE_SIG_KEY_TYPE_ECDSA \
                     ...
                     -e CONFIG_LOCK_DOWN_KERNEL_FORCE_INTEGRITY
}

_dori_pkgbuild_end_hook() {
    source+=("${_dori_add_source[@]}")
    #b2sums+=("${_dori_add_b2sums[@]}") keep this until we can stop touching real checksums in the main PKGBUILDs
}

_dori_set_optimizations() {
    case $1 in
        client)
            _processor_opt=zen4
            ;;
        server)
            ...
            ;;
        *)
            echo "Invalid optimizations type $1"
            exit 1
            ;;
    esac
}
